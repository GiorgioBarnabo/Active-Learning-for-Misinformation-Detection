...
def load_all_ordered(path, warm_start_years, training_years):
    all_loaded_data = {}
    all_eval_data = {}
...
def merge_new_data(data, new_x, new_y, train_val_test_split_ratio,
                   ...
                   train_last_samples, val_last_samples, add_val_to_train): #Active Learning Method
...
def main():
    ...
    results_set = 'offline_fulltrain_fullval_without-warmstart' #specify characteristics current set of experiments
    ...
    training_years = [2017,2019] #to train (after warm-start) from year[0] (included) to year[1] (included)

    offline_AL = 36 #0 == Online, >0 == Number of iterations for Offline AL
    ...
    all_year_month_ordered_keys, all_data, all_year_month_test_ordered_keys, all_test_data = load_all_ordered(os.path.join(project_folder, 'data', 'features', data_opt), warm_start_years, training_years) #to load all data
    ...
    warm_start_starting_key_id,warm_start_ending_key_id = get_warm_start_key_ids(all_year_month_ordered_keys, warm_start_years)
                ...
                if not os.path.isfile(results_filename):
                    ...
                    for sample in range(nb_sample):
                        ...
                        for iteration_num,(starting_key_id,current_key_id) in enumerate(iteration_ranges):
                            ...
                            data, (new_x, new_y), (new_positives, new_negatives) = merge_new_data(data, new_x, new_y, train_val_test_split_ratio,
                            ...
                            if new_x is not None:
                                print("REM SHAPES",new_x.shape, new_y.shape)

                            model_folder = os.path.join(project_folder, 'src', 'models', 'early_detection_with_RNN_and_CNN', 'rc_model', results_set, data_opt)
