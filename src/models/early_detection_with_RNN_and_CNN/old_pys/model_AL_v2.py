...
def model_train(model, file_name, data, epochs, batch_size, iteration_num = 0):
    ...
    history = model.fit(input_train, output_train, epochs=epochs, batch_size=batch_size, validation_data = (input_valid, output_valid), callbacks=[call_back], verbose = 0)
...
def merge_new_data(data, new_x, new_y, split_ratio, AL_method, model, num_urls_k, combined_AL_nums, diversity_nums): #Active Learning Method
    if data['x_train'] is None:
        ...
        if new_x.shape[0]<=num_urls_k: #x
            ...
            if AL_method == "random":
                new_ids = AL_random(new_x, new_y, take_until)
            elif AL_method == "uncertainty-margin":
                new_ids = AL_uncertainty_margin(new_x, new_y, model, take_until)
            elif AL_method == "diversity-cluster":
                new_ids = AL_diversity_cluster(new_x, new_y, model, take_until, diversity_nums)
            elif AL_method == "combined":
                new_ids1 = AL_uncertainty_margin(new_x, new_y, model, take_until)
                new_ids2 = AL_diversity_cluster(new_x, new_y, model, take_until, diversity_nums)
                
                new_ids12 = [new_ids1, new_ids2]
                ...
                while len(new_ids)<take_until:
                    poss = new_ids12[i][i12[i]:(i12[i]+combined_AL_nums[i])]
                    new_ids.update(dict(zip(poss,[None]*len(poss))))

                    i12[i] += combined_AL_nums[i]
                    i = (i+1)%2
                    print(new_ids.keys())
...
def split_by_ratio(new_x, new_y, split_ratio):
    app = int(new_x.shape[0] * split_ratio[0])
...
def AL_random(new_x, new_y, take_until):
    app = np.array(range(take_until))
    return app

def AL_uncertainty_margin(new_x, new_y, model, take_until):
...
def AL_diversity_cluster(new_x, new_y, model, take_until, diversity_nums):
...
def main():
    ...
    AL_methods = ["random", "combined", "uncertainty-margin", "diversity-cluster"] #don't use "_" to keep filename easily separable
    #"random"
    #"uncertainty-margin"
    #"diversity-cluster"
    #"combined"
    ...
    combined_AL_nums = [1,1] #ignored if AL_method is not "combined"
            ...
            for num_urls_k in num_urls_k_list:
                ...
                for sample in range(nb_sample):
                    ...
                    for iteration_num,current_key_id in enumerate(range(warm_start_key_id,len(all_year_month_ordered_keys)+1)):
                        ...
                        data = merge_new_data(data, new_x, new_y, split_ratio, AL_method, model, num_urls_k, combined_AL_nums, diversity_nums)
