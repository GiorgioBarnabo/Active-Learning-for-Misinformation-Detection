...
"""
def initialize_graph_model(model_parameters,seed):
	torch.manual_seed(seed)
	if torch.cuda.is_available():
		torch.cuda.manual_seed(seed)

	model = Model(model_parameters.graph_args, concat=model_parameters.graph_args.concat)
	if model_parameters.graph_args.multi_gpu:
		model = DataParallel(model)
	model = model.to(model_parameters.graph_args.device)
	return model


def train_graph_model(model_parameters, data):
	optimizer = torch.optim.Adam(model.parameters(), lr=model_parameters.graph_args.lr, weight_decay=model_parameters.graph_args.weight_decay)
